...
func koImg(service string, v domain.ServiceVersion) (name.Reference, error) {
	...
	b, err := build.NewGo(ctx, ".",
		...
		build.WithBaseImages(func(ctx context.Context, _ string) (name.Reference, build.Result, error) {
			ref := name.MustParseReference("cgr.dev/chainguard/static:latest")
	...
	p, err := publish.NewDaemon(
		imageNamer,
		[]string{"latest"},
		publish.WithLocalDomain("ko.local"),
	)
	if err != nil {
		return nil,
